...
import * as types from "../data/proto/game";
import protobufjs from "protobufjs";
import { CmdID, DataPacket, PacketName } from "../kcp/packet"
...
export default class ProtoFactory {
    ...
    static getBody(packet: DataPacket) {
        const id = packet.id;
        const data = packet.data

        const message = this.getType(CmdID[id] as PacketName)
        const body = message.decode(data)
